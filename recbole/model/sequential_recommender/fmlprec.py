...
class FMLPRec(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        for i in range(self.num_layers):
            item_emb = self.filter_layers[i](item_emb)
...
class FilterLayer(nn.Module):
    def __init__(self, d_model, dropout, seq_len):
        ...
        self.complex_weight = nn.Parameter(torch.randn(1, seq_len//2 + 1, d_model, 2, dtype=torch.float32) * 0.02)
    ...
    def forward(self, input_tensor):
        ...
        batch, seq_len, d_model = input_tensor.shape
