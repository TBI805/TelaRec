...
class TransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        gsp = self.multi_head_attention(hidden_states, attention_mask)
        layer_output = self.alpha * dsp + ( 1 - self.alpha ) * gsp
...
class Dynamic_Spectral_Gate(nn.Module):
    ...
    def forward(self, x_in):
        ...
        # x_fft = self.freq_conv(x_fft)  # [batch, freq_bins, input_size]
        # x_fft = F.gelu(self.LayerNorm(x_fft))
        weight = torch.view_as_complex(self.complex_weight)
        # weight = weight / (weight.abs().norm(dim=1, keepdim=True) + 1e-6)
        x_weighted = x_fft * weight

        if self.adaptive_filter is True:
